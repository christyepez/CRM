...
public sealed class OpportunityPipelinePolicyTests
{
    ...
    [Fact] public void Progress_RejectsSkippingIntermediateStage() { var stage3=Guid.NewGuid().ToString(); var stages=new[]{new OpportunityPipelineStageDefinition(Stage1,"Discovery",1),new OpportunityPipelineStageDefinition(Stage2,"Proposal",2),new OpportunityPipelineStageDefinition(stage3,"Commit",3)}; var r=OpportunityPipelinePolicy.Evaluate(Command(OpportunityPipelineOperation.Progress, stageId:stage3, stages:stages, existing:Snapshot())); Assert.False(r.Success); Assert.False(r.Changed); Assert.Equal(OpportunityPipelineErrorCode.InvalidStageProgression,r.ErrorCode); }
    ...
    [Fact] public void TerminalOpportunity_CannotBeUpdatedOrProgressed() { var update=OpportunityPipelinePolicy.Evaluate(Command(OpportunityPipelineOperation.Update, existing:Snapshot(OpportunityStatus.Won,100))); var progress=OpportunityPipelinePolicy.Evaluate(Command(OpportunityPipelineOperation.Progress, stageId:Stage2, existing:Snapshot(OpportunityStatus.Cancelled))); Assert.Equal(OpportunityPipelineErrorCode.TerminalOpportunityCannotBeModified,update.ErrorCode); Assert.Equal(OpportunityPipelineErrorCode.CancelledOpportunityCannotBeChanged,progress.ErrorCode); Assert.False(update.Changed); Assert.False(progress.Changed); }
