...
public sealed class OpportunityManagementServiceTests
{
    ...
    [Fact] public async Task Progress_SameOrSkippedStage_WritesZero(){var s=Seeded();var service=new OpportunityManagementService(s);var x=s.Items.Single();var same=await service.ProgressAsync(x.Id,new(Stage1,Stages));var stage3=Guid.NewGuid().ToString();var stages=new[]{new OpportunityPipelineStageDefinition(Stage1,"Discovery",1),new OpportunityPipelineStageDefinition(Stage2,"Proposal",2),new OpportunityPipelineStageDefinition(stage3,"Commit",3)};var skipped=await service.ProgressAsync(x.Id,new(stage3,stages));Assert.False(same.Success);Assert.False(skipped.Success);Assert.Equal(OpportunityPipelineErrorCode.InvalidStageProgression.ToString(),same.ErrorCode);Assert.Equal(OpportunityPipelineErrorCode.InvalidStageProgression.ToString(),skipped.ErrorCode);Assert.Equal(0,s.SaveCount);}
    [Fact] public async Task TerminalOpportunity_UpdateAndProgress_WriteZero(){var s=Seeded(OpportunityStatus.Won,100);var service=new OpportunityManagementService(s);var x=s.Items.Single();var update=await service.UpdateAsync(x.Id,Update(expected:2000));var progress=await service.ProgressAsync(x.Id,new(Stage2,Stages));Assert.False(update.Success);Assert.False(progress.Success);Assert.Equal(0,s.SaveCount);}
    ...
    [Fact] public async Task CrossTerminalAction_WritesZero(){var s=Seeded(OpportunityStatus.Lost);var service=new OpportunityManagementService(s);var x=s.Items.Single();var r=await service.WinAsync(x.Id);Assert.False(r.Success);Assert.Equal(OpportunityPipelineErrorCode.LostOpportunityCannotBeWon.ToString(),r.ErrorCode);Assert.Equal(0,s.SaveCount);}
