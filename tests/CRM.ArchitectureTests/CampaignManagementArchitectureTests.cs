...
public sealed class CampaignManagementArchitectureTests
{
    ...
    [Fact]
    public void CampaignRuntimeRoutes_RemainFoundationOnlyAsSprintAdvances()
    {
        var root = FindRepositoryRoot();
        var program = File.ReadAllText(Path.Combine(root, "src", "CRM.Api", "Program.cs"));
        Assert.Contains("MapGet(\"/api/crm/foundation/campaigns", program, StringComparison.OrdinalIgnoreCase);
        foreach (var marker in new[] { "MapGet(\"/api/crm/campaigns", "MapPost(\"/api/crm/campaigns", "MapPut(\"/api/crm/campaigns", "MapDelete(\"/api/crm/campaigns", "MapDelete(\"/api/crm/foundation/campaigns" })
