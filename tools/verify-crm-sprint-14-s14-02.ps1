...
foreach($m in @("S1402Decision: Implemented","PersistenceClassification: FoundationOnly / NonProductionSeam","DomainRulesDuplicatedInApplication: false","NoChangePersistenceSuppressed: true","ProductiveOpportunityRouteEnabled: false","FoundationOpportunityRouteEnabled: false","PortalRuntimeEnabled: false","CommonDbRuntimeEnabled: false","SimulatedProductionTouched: false")){if(-not$doc.Contains($m)){throw "Missing S14-02 doc marker $m"}}
foreach($m in @("CRM Sprint 14 S14-03 - Opportunity Foundation API","Sprint 14 S14-02 merge commit required","codex/prompts/sprint-14-opportunity-pipeline-s14-03.md")){if(-not$n.Contains($m)){throw "Missing next marker $m"}}
foreach($m in @("/api/crm/opportunities","/api/crm/foundation/opportunities")){if($program.Contains($m)){throw "Forbidden Opportunity route in S14-02: $m"}}
