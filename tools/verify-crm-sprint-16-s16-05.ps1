...
foreach($m in @('/api/crm/foundation/accounts','AccountManagementPolicy.Evaluate','IAccountFoundationStore','maxlength="160"','maxlength="64"','maxlength="120"','maxlength="80"','Activate account','Deactivate account')){if(-not$arch.Contains($m)){throw "Missing S16-05 architecture marker: $m"}}
...
if(-not(($next -match 'CRM Sprint 16 S16-0[6-7]') -and ($next -match 'codex/prompts/sprint-16-account-management-s16-0[6-7]\.md'))){throw 'Invalid Sprint 16 S16-05 forward handoff.'}
