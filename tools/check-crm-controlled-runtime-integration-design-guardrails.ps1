...
foreach ($file in $changedFiles) {
    if ($file.FullName -notmatch "\\(docs|tools|codex)\\" -and $file.Name -ne "TASKS.md") {
        Fail "P4 must only modify docs, tools or codex: $($file.FullName)"
    }
}
...
if ($changedText -match "https://(?!github\.com)|http://(?!localhost|127\.0\.0\.1)") {
    Fail "Possible private or real URL found in P4 changed files."
}

if ($changedText -match "localStorage|sessionStorage") {
    Fail "Browser token storage marker found in P4 changed files."
}
...
if ($changedText -match "AddAuthentication|UseAuthentication|UseAuthorization|AuthorizeAttribute|JwtBearer|CookieAuthentication|SqlConnection\(|UseSqlServer\(|AddDbContext\(|MigrationBuilder") {
    Fail "Runtime Auth, DB or migration marker found in P4 changed files."
}

if ($failures.Count -gt 0) { exit 1 }

Pass "CRM controlled runtime integration design guardrails passed."
exit 0
