...
foreach($f in @('MapGet("/api/crm/cases','MapPost("/api/crm/cases','MapPut("/api/crm/cases','MapDelete("/api/crm/cases','MapGet("/api/crm/foundation/cases','MapPost("/api/crm/foundation/cases','MapPut("/api/crm/foundation/cases','MapDelete("/api/crm/foundation/cases')){if($program.Contains($f)){throw "Forbidden S18-01 Case route detected: $f"}}
if(-not($next.Contains('CRM Sprint 18 S18-02 - Case Application Service and Foundation Store') -and $next.Contains('codex/prompts/sprint-18-case-management-s18-02.md'))){throw 'Invalid S18-02 handoff.'}
