...
foreach ($file in $changedFiles) {
    if ($file.FullName -notmatch "\\(docs|tools|codex)\\" -and $file.Name -ne "TASKS.md") {
        Fail "P8 must only modify docs, tools or codex: $($file.FullName)"
    }
}
...
$contentFiles = $changedFiles | Where-Object { $_.Name -notin @(
    "check-crm-controlled-runtime-pilot-enablement-dry-run-guardrails.ps1"
) }
...
if ($changedText -match "https://|http://") {
    Fail "URL found in P8 changed content."
}

if ($changedText -match "localStorage|sessionStorage") {
    Fail "Browser token storage marker found in P8 changed content."
}
...
if ($changedText -match "AddAuthentication|UseAuthentication|UseAuthorization|AuthorizeAttribute|JwtBearer|CookieAuthentication|SqlConnection\(|UseSqlServer\(|AddDbContext\(|MigrationBuilder") {
    Fail "Runtime Auth, DB or migration marker found in P8 changed content."
}

if ($changedText -notmatch "DryRunOnly") {
    Fail "DryRunOnly evidence marker is required in P8 changed content."
}

if ($failures.Count -gt 0) { exit 1 }

Pass "CRM controlled runtime pilot enablement dry run guardrails passed."
exit 0
