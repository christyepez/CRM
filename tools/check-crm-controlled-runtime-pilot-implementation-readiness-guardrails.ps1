...
foreach ($file in $changedFiles) {
    if ($file.FullName -notmatch "\\(docs|tools|codex)\\" -and $file.Name -ne "TASKS.md") {
        Fail "P12 must only modify docs, tools or codex: $($file.FullName)"
    }
}
...
$contentFiles = $changedFiles | Where-Object { $_.Name -notin @(
    "check-crm-controlled-runtime-pilot-implementation-readiness-guardrails.ps1"
) }
...
if ($changedText -match "https://|http://") {
    Fail "URL found in P12 changed content."
}

if ($changedText -match "localStorage|sessionStorage") {
    Fail "Browser token storage marker found in P12 changed content."
}
...
if ($changedText -match "AddAuthentication|UseAuthentication|UseAuthorization|AuthorizeAttribute|JwtBearer|CookieAuthentication|SqlConnection\(|UseSqlServer\(|AddDbContext\(|MigrationBuilder") {
    Fail "Runtime Auth, DB or migration marker found in P12 changed content."
}

if ($changedText -notmatch "ReadinessReviewOnly") {
    Fail "ReadinessReviewOnly evidence marker is required in P12 changed content."
}

if ($changedText -match "ConditionalFutureGoExecuted:\s*true") {
    Fail "ConditionalFutureGo must not be executed in P12."
}
...
if ($changedText -match "RuntimePortalCallsEnabled:\s*true|RuntimePortalCouplingEnabled:\s*true|CommonDbRuntimeEnabled:\s*true|ProductivePortalNavigationEnabled:\s*true|ProductivePortalGatewayRoutesEnabled:\s*true") {
    Fail "Runtime enablement marker must remain false in P12."
}

if ($failures.Count -gt 0) { exit 1 }

Pass "CRM controlled runtime pilot implementation readiness guardrails passed."
exit 0
