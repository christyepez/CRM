...
foreach ($file in $changedFiles) {
    if ($file.FullName -notmatch "\\(docs|tools|codex)\\" -and $file.Name -ne "TASKS.md") {
        Fail "P11 must only modify docs, tools or codex: $($file.FullName)"
    }
}
...
$contentFiles = $changedFiles | Where-Object { $_.Name -notin @(
    "check-crm-controlled-runtime-pilot-conditional-implementation-guardrails.ps1"
) }
...
$unsafeSecretLines = $contentFiles | Select-String -Pattern "(client_secret|token|secret|Password|Pwd)\s*[:=]\s*['""][^'""]{8,}" -ErrorAction SilentlyContinue |
    Where-Object { $_.Line -notmatch "(false|true|placeholder|logical|metadata|redacted|example|no real|not configured|SecretProvider|Secret Provider|secret-provider|SecretsPresent|RealSecretProviderConfigured|logical name|marker)" }
if ($unsafeSecretLines) { Fail "Possible real secret/token value found." }

if ($changedText -match "https://|http://") {
    Fail "URL found in P11 changed content."
}

if ($changedText -match "localStorage|sessionStorage") {
    Fail "Browser token storage marker found in P11 changed content."
}
...
if ($changedText -match "AddAuthentication|UseAuthentication|UseAuthorization|AuthorizeAttribute|JwtBearer|CookieAuthentication|SqlConnection\(|UseSqlServer\(|AddDbContext\(|MigrationBuilder") {
    Fail "Runtime Auth, DB or migration marker found in P11 changed content."
}

if ($changedText -notmatch "ImplementationPlanOnly") {
    Fail "ImplementationPlanOnly evidence marker is required in P11 changed content."
}

if ($changedText -match "ConditionalFutureGoExecuted:\s*true") {
    Fail "ConditionalFutureGo must not be executed in P11."
}
...
if ($changedText -match "RuntimePortalCallsEnabled:\s*true|RuntimePortalCouplingEnabled:\s*true|CommonDbRuntimeEnabled:\s*true|ProductivePortalNavigationEnabled:\s*true|ProductivePortalGatewayRoutesEnabled:\s*true") {
    Fail "Runtime enablement marker must remain false in P11."
}

if ($failures.Count -gt 0) { exit 1 }

Pass "CRM controlled runtime pilot conditional implementation guardrails passed."
exit 0
