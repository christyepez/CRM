    ...
    CrmFoundationPermissionGuard permissionGuard)
{
    ...
    private static FoundationContactResponse ToResponse(CrmFoundationPreviewItemContract item) =>
        new(item.Id, item.DisplayName.Split(' ', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault(), item.DisplayName.Split(' ', StringSplitOptions.RemoveEmptyEntries).Skip(1).FirstOrDefault(), item.Metadata.GetValueOrDefault("email"), item.Metadata.GetValueOrDefault("phone"), item.Metadata.GetValueOrDefault("title"), item.Metadata.GetValueOrDefault("accountId"), Enum.TryParse<CRM.Domain.Enums.PreferredContactMethod>(item.Metadata.GetValueOrDefault("preferredContactMethod"), true, out var parsed) ? parsed : CRM.Domain.Enums.PreferredContactMethod.NotSpecified, item.Status, true, "NonProductionSeam", false, false, false, "FoundationSimulation", FoundationCrudSupport.WarningText);
}
