    ...
    CrmFoundationPermissionGuard permissionGuard)
{
    ...
    private static FoundationContactResponse FromUpdate(string id, FoundationContactUpdateRequest request, CrmFoundationPreviewItemContract? existing) =>
        new(id, FoundationCrudSupport.CleanOptional(request.FirstName), FoundationCrudSupport.CleanOptional(request.LastName), FoundationCrudSupport.CleanOptional(request.Email), FoundationCrudSupport.CleanOptional(request.Phone), FoundationCrudSupport.CleanOptional(request.Title), FoundationCrudSupport.CleanOptional(request.AccountId), request.PreferredContactMethod, FoundationCrudSupport.Clean(request.Status, existing?.Status ?? "PreviewOnly"), true, "NonProductionSeam", false, false, false, "FoundationSimulation", FoundationCrudSupport.WarningText);

    private static CrmFoundationPreviewItemContract ToPreview(FoundationContactResponse response) =>
        FoundationCrudSupport.ToPreview(response.Id, "Contact", FoundationCrudSupport.Clean($"{response.FirstName} {response.LastName}", "Contact Foundation Preview"), response.Status, new Dictionary<string, string> { ["email"] = response.Email ?? "", ["phone"] = response.Phone ?? "", ["title"] = response.Title ?? "", ["accountId"] = response.AccountId ?? "", ["preferredContactMethod"] = response.PreferredContactMethod.ToString() });

    private static FoundationContactResponse ToResponse(CrmFoundationPreviewItemContract item) =>
        new(item.Id, item.DisplayName.Split(' ', StringSplitOptions.RemoveEmptyEntries).FirstOrDefault(), item.DisplayName.Split(' ', StringSplitOptions.RemoveEmptyEntries).Skip(1).FirstOrDefault(), item.Metadata.GetValueOrDefault("email"), item.Metadata.GetValueOrDefault("phone"), item.Metadata.GetValueOrDefault("title") ?? item.Metadata.GetValueOrDefault("role"), item.Metadata.GetValueOrDefault("accountId"), Enum.TryParse<CRM.Domain.Enums.PreferredContactMethod>(item.Metadata.GetValueOrDefault("preferredContactMethod"), true, out var parsed) ? parsed : CRM.Domain.Enums.PreferredContactMethod.NotSpecified, item.Status, true, "NonProductionSeam", false, false, false, "FoundationSimulation", FoundationCrudSupport.WarningText);
}
