...
## .NET 10 API candidate checkpoint

`tools/CRM.Api.SqlCandidate` now resolves the framework integration constraint by linking existing API source into a separate .NET 10 host. Native API target remains .NET 8. Shared Program contains only two compile-conditional composition hooks enabled in the candidate project. The candidate replaces all 13 store registrations with SQL adapters, requires explicit Development configuration and a tenant/dedicated encrypted non-SA SQL connection, and binds only to loopback on an ephemeral port. It never seeds/migrates automatically or falls back to memory. Certificate bypass is permitted only for the explicit loopback lab SQL endpoint; username checks do not replace a DB privilege review.

Every candidate HTTP surface is locked with sanitized 503/no-store except GET `/health` and GET `/health/live`. GET `/health/ready` is deliberately blocked. This is a composition/test host, NOT activated durable CRM business runtime or completed Portal authorization/audit integration. See `docs/architecture/adr-20260918-crm-net10-sql-candidate-host.md`.

Implementation checkpoint `29b9bd06bf5c82aa15f23417169cf4fb1da75009`: 24/24 candidate configuration/DI/HTTP tests passed on both machines, with 100% line / 96.42% branch coverage for the new composition class and 100% / 100% for its HTTP middleware state machine. These are new-code metrics, not full linked-API coverage. Native .NET 8 regression at that same commit: 733 Unit + 180 Architecture passed on BOTH machines, demonstrating the conditional hooks preserve current API behavior. Candidate tests use synthetic nonexistent SQL credentials and do not open a connection. A transient MarketingIndo connector 504 was resolved by read-only state inspection before retry; no duplicate live-service operation occurred. No Docker/Compose runtime was deployed or source CRM restarted.

Pending integration: Portal permissions/audit; a faithful LIVE export and validated import into `crm.FoundationRecords` (not only the migration snapshot table); verification against the active source artifact and dirty-source contract differences; transactional/reference/lifecycle review; complete runtime readiness and controlled cutover. The candidate still deliberately blocks readiness and every CRM route.

Additional checks at `29b9bd0`: 48/48 migration/SQL-adapter regression tests passed on BOTH machines. Total executed suites per machine: 24 candidate + 48 migration/adapters + 733 native Unit + 180 native Architecture = 985 tests, all passed, none skipped. Official NuGet advisory audit of the candidate test project including transitive dependencies exited 0 on MarketingIndo and reported no vulnerable packages in the current feed. This is not a substitute for security review or full-system coverage. No CI/image/release gate is claimed from local tests.
